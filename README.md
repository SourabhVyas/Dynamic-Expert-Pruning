...
### Dataset:
To get the dataset, refer to `./dataset/build_dataset.ipynb`, it contains code to download the dataset to drive.


### Training

To train, need to run the following in a sequence:
```bash
cd ./trainer
python train_pretrain.py --data_path ../dataset/data/pretrain_t2t_mini.jsonl --use_moe 1 --batch_size 128
python train_full_sft.py --data_path ../dataset/data/sft_t2t_mini.jsonl --use_moe 1 --batch_size 128
```
...
- Script: ./benchmark.py
- Prompt set: ./benchmark/fixed_prompts.jsonl
...
python benchmark.py \
  ...
  --checkpoint ./out/expert_dist_pruned_768_moe.pth \
...
#### Note
- Prompt file is JSONL (one case per line): `id`, `prompt`, `match_type`, and `expected/answer`.

#### Output
Each run saves:
- Console per-case line (example):
  `[45/50] code_bug_fix(avg of 5 runs) | base=30% | pruned=20% | fidelity=0.017 | speedup=1.05x`
- Console summary metrics:
  - `Base Accuracy`
  - `Pruned Accuracy`
  - `Accuracy Delta (pruned-base)`
  - `Average Fidelity`
  - `Average Speedup`
- Full per-case details are exported to JSON + CSV under `./benchmark/reports`, including:
  - correctness for base/pruned (`base_correct`, `pruned_correct`)
  - quality similarity (`fidelity`)
  - efficiency comparison (`speedup`, `base_latency_sec`, `pruned_latency_sec`)
  - raw outputs (`base_response`, `pruned_response`)
